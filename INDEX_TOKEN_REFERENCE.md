...
- `users_1ENTK` - User accounts (includes `primary_role_id` for optimized role lookup)
...
To `users_{INDEX_TOKEN}`:

- `primary_role_id UUID REFERENCES roles_{INDEX_TOKEN}(id) ON DELETE SET NULL` - Direct reference to user's primary role for optimized login (eliminates extra `user_roles` query)
...
_Contact Nexverse for implementation details._
