...
CREATE INDEX idx_roles_code ON roles_5HKSK(role_code);
CREATE INDEX idx_roles_active ON roles_5HKSK(is_active);
CREATE INDEX idx_roles_system ON roles_5HKSK(is_system_role);

COMMENT ON TABLE roles_5HKSK IS 'Dynamic role management - schools can create custom roles';
COMMENT ON COLUMN roles_5HKSK.is_system_role IS 'System roles (student, teacher, parent, admin) cannot be deleted';

-- 1.2 Modules (Feature Grouping)
CREATE TABLE modules_5HKSK (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  module_code VARCHAR(50) UNIQUE NOT NULL,
  module_name VARCHAR(100) NOT NULL,
  parent_module_id UUID REFERENCES modules_5HKSK(id) ON DELETE SET NULL,
  description TEXT,
...
CREATE INDEX idx_modules_code ON modules_5HKSK(module_code);
CREATE INDEX idx_modules_parent ON modules_5HKSK(parent_module_id);
CREATE INDEX idx_modules_active ON modules_5HKSK(is_active);
CREATE INDEX idx_modules_order ON modules_5HKSK(display_order);

COMMENT ON TABLE modules_5HKSK IS 'Module hierarchy for organizing permissions';
COMMENT ON COLUMN modules_5HKSK.route_prefix IS 'Frontend route prefix for this module';

-- 1.3 Permissions (Granular Actions)
CREATE TABLE permissions_5HKSK (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  module_id UUID NOT NULL REFERENCES modules_5HKSK(id) ON DELETE CASCADE,
...
CREATE INDEX idx_permissions_module ON permissions_5HKSK(module_id);
CREATE INDEX idx_permissions_code ON permissions_5HKSK(permission_code);
CREATE INDEX idx_permissions_resource ON permissions_5HKSK(resource_type, resource_path);
CREATE INDEX idx_permissions_active ON permissions_5HKSK(is_active);

COMMENT ON TABLE permissions_5HKSK IS 'Granular permissions - specific actions within modules';
COMMENT ON COLUMN permissions_5HKSK.resource_type IS 'Type: api, route, ui_component';

-- 1.4 Role-Permission Mapping
CREATE TABLE role_permissions_5HKSK (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  role_id UUID NOT NULL REFERENCES roles_5HKSK(id) ON DELETE CASCADE,
  permission_id UUID NOT NULL REFERENCES permissions_5HKSK(id) ON DELETE CASCADE,
...
CREATE INDEX idx_role_permissions_role ON role_permissions_5HKSK(role_id);
CREATE INDEX idx_role_permissions_permission ON role_permissions_5HKSK(permission_id);

COMMENT ON TABLE role_permissions_5HKSK IS 'Maps permissions to roles with CRUD + Approve + Export actions';
COMMENT ON COLUMN role_permissions_5HKSK.constraints IS 'JSON constraints like amount limits, department access';
...
CREATE TABLE users_5HKSK (
  ...
  primary_role_id UUID REFERENCES roles_5HKSK(id) ON DELETE SET NULL,
  is_active BOOLEAN DEFAULT true,
...
CREATE INDEX idx_users_primary_role ON users_5HKSK(primary_role_id) WHERE deleted_at IS NULL;

COMMENT ON TABLE users_5HKSK IS 'User management table - primary_role_id for quick access, additional roles via user_roles_5HKSK';
COMMENT ON COLUMN users_5HKSK.auth_user_id IS 'Links to Supabase auth.users table';
COMMENT ON COLUMN users_5HKSK.primary_role_id IS 'Primary role for quick access during login - synced with user_roles_5HKSK';

-- 1.6 User-Role Assignment
CREATE TABLE user_roles_5HKSK (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES users_5HKSK(id) ON DELETE CASCADE,
  role_id UUID NOT NULL REFERENCES roles_5HKSK(id) ON DELETE RESTRICT,
  is_primary BOOLEAN DEFAULT true,
  assigned_by UUID REFERENCES users_5HKSK(id) ON DELETE SET NULL,
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW(),
  UNIQUE(user_id, role_id)
);

CREATE INDEX idx_user_roles_user ON user_roles_5HKSK(user_id);
CREATE INDEX idx_user_roles_role ON user_roles_5HKSK(role_id);
CREATE INDEX idx_user_roles_primary ON user_roles_5HKSK(user_id, is_primary) WHERE is_primary = true;

COMMENT ON TABLE user_roles_5HKSK IS 'Assigns roles to users - only admin can modify';
COMMENT ON COLUMN user_roles_5HKSK.is_primary IS 'One primary role per user';

-- 1.7 Additional Permissions (Cross-Role Access)
CREATE TABLE user_additional_permissions_5HKSK (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES users_5HKSK(id) ON DELETE CASCADE,
  permission_id UUID NOT NULL REFERENCES permissions_5HKSK(id) ON DELETE CASCADE,
  granted_by UUID NOT NULL REFERENCES users_5HKSK(id) ON DELETE SET NULL,
...
CREATE INDEX idx_user_additional_permissions_user ON user_additional_permissions_5HKSK(user_id);
CREATE INDEX idx_user_additional_permissions_permission ON user_additional_permissions_5HKSK(permission_id);
CREATE INDEX idx_user_additional_permissions_active ON user_additional_permissions_5HKSK(user_id, is_active) WHERE is_active = true;

COMMENT ON TABLE user_additional_permissions_5HKSK IS 'Cross-role permissions granted by admin - permanent until revoked';
COMMENT ON COLUMN user_additional_permissions_5HKSK.reason IS 'Required justification for granting permission';

-- 1.8 Permission Resolution Function
CREATE OR REPLACE FUNCTION get_user_permissions_5HKSK(p_user_id UUID)
RETURNS TABLE (
  permission_id UUID,
...
BEGIN
  ...
  SELECT DISTINCT
    ...
    rp.constraints
  FROM user_roles_5HKSK ur
  JOIN roles_5HKSK r ON ur.role_id = r.id
  JOIN role_permissions_5HKSK rp ON r.id = rp.role_id
  JOIN permissions_5HKSK p ON rp.permission_id = p.id
  JOIN modules_5HKSK m ON p.module_id = m.id
  ...
  SELECT DISTINCT
    ...
    uap.constraints
  FROM user_additional_permissions_5HKSK uap
  JOIN permissions_5HKSK p ON uap.permission_id = p.id
  JOIN modules_5HKSK m ON p.module_id = m.id
...
COMMENT ON FUNCTION get_user_permissions_5HKSK IS 'Returns all effective permissions for a user from role and additional permissions';
