...
CREATE INDEX idx_roles_code ON roles_4CBV(role_code);
CREATE INDEX idx_roles_active ON roles_4CBV(is_active);
CREATE INDEX idx_roles_system ON roles_4CBV(is_system_role);

COMMENT ON TABLE roles_4CBV IS 'Dynamic role management - schools can create custom roles';
COMMENT ON COLUMN roles_4CBV.is_system_role IS 'System roles (student, teacher, parent, admin) cannot be deleted';

-- 1.2 Modules (Feature Grouping)
CREATE TABLE modules_4CBV (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  module_code VARCHAR(50) UNIQUE NOT NULL,
  module_name VARCHAR(100) NOT NULL,
  parent_module_id UUID REFERENCES modules_4CBV(id) ON DELETE SET NULL,
  description TEXT,
...
CREATE INDEX idx_modules_code ON modules_4CBV(module_code);
CREATE INDEX idx_modules_parent ON modules_4CBV(parent_module_id);
CREATE INDEX idx_modules_active ON modules_4CBV(is_active);
CREATE INDEX idx_modules_order ON modules_4CBV(display_order);

COMMENT ON TABLE modules_4CBV IS 'Module hierarchy for organizing permissions';
COMMENT ON COLUMN modules_4CBV.route_prefix IS 'Frontend route prefix for this module';

-- 1.3 Permissions (Granular Actions)
CREATE TABLE permissions_4CBV (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  module_id UUID NOT NULL REFERENCES modules_4CBV(id) ON DELETE CASCADE,
...
CREATE INDEX idx_permissions_module ON permissions_4CBV(module_id);
CREATE INDEX idx_permissions_code ON permissions_4CBV(permission_code);
CREATE INDEX idx_permissions_resource ON permissions_4CBV(resource_type, resource_path);
CREATE INDEX idx_permissions_active ON permissions_4CBV(is_active);

COMMENT ON TABLE permissions_4CBV IS 'Granular permissions - specific actions within modules';
COMMENT ON COLUMN permissions_4CBV.resource_type IS 'Type: api, route, ui_component';

-- 1.4 Role-Permission Mapping
CREATE TABLE role_permissions_4CBV (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  role_id UUID NOT NULL REFERENCES roles_4CBV(id) ON DELETE CASCADE,
  permission_id UUID NOT NULL REFERENCES permissions_4CBV(id) ON DELETE CASCADE,
...
CREATE INDEX idx_role_permissions_role ON role_permissions_4CBV(role_id);
CREATE INDEX idx_role_permissions_permission ON role_permissions_4CBV(permission_id);

COMMENT ON TABLE role_permissions_4CBV IS 'Maps permissions to roles with CRUD + Approve + Export actions';
COMMENT ON COLUMN role_permissions_4CBV.constraints IS 'JSON constraints like amount limits, department access';
...
CREATE TABLE users_4CBV (
  ...
  primary_role_id UUID REFERENCES roles_4CBV(id) ON DELETE SET NULL,
  is_active BOOLEAN DEFAULT true,
...
CREATE INDEX idx_users_primary_role ON users_4CBV(primary_role_id) WHERE deleted_at IS NULL;

COMMENT ON TABLE users_4CBV IS 'User management table - primary_role_id for quick access, additional roles via user_roles_4CBV';
COMMENT ON COLUMN users_4CBV.auth_user_id IS 'Links to Supabase auth.users table';
COMMENT ON COLUMN users_4CBV.primary_role_id IS 'Primary role for quick access during login - synced with user_roles_4CBV';

-- 1.6 User-Role Assignment
CREATE TABLE user_roles_4CBV (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES users_4CBV(id) ON DELETE CASCADE,
  role_id UUID NOT NULL REFERENCES roles_4CBV(id) ON DELETE RESTRICT,
  is_primary BOOLEAN DEFAULT true,
  assigned_by UUID REFERENCES users_4CBV(id) ON DELETE SET NULL,
  created_at TIMESTAMP DEFAULT NOW(),
  updated_at TIMESTAMP DEFAULT NOW(),
  UNIQUE(user_id, role_id)
);

CREATE INDEX idx_user_roles_user ON user_roles_4CBV(user_id);
CREATE INDEX idx_user_roles_role ON user_roles_4CBV(role_id);
CREATE INDEX idx_user_roles_primary ON user_roles_4CBV(user_id, is_primary) WHERE is_primary = true;

COMMENT ON TABLE user_roles_4CBV IS 'Assigns roles to users - only admin can modify';
COMMENT ON COLUMN user_roles_4CBV.is_primary IS 'One primary role per user';

-- 1.7 Additional Permissions (Cross-Role Access)
CREATE TABLE user_additional_permissions_4CBV (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES users_4CBV(id) ON DELETE CASCADE,
  permission_id UUID NOT NULL REFERENCES permissions_4CBV(id) ON DELETE CASCADE,
  granted_by UUID NOT NULL REFERENCES users_4CBV(id) ON DELETE SET NULL,
...
CREATE INDEX idx_user_additional_permissions_user ON user_additional_permissions_4CBV(user_id);
CREATE INDEX idx_user_additional_permissions_permission ON user_additional_permissions_4CBV(permission_id);
CREATE INDEX idx_user_additional_permissions_active ON user_additional_permissions_4CBV(user_id, is_active) WHERE is_active = true;

COMMENT ON TABLE user_additional_permissions_4CBV IS 'Cross-role permissions granted by admin - permanent until revoked';
COMMENT ON COLUMN user_additional_permissions_4CBV.reason IS 'Required justification for granting permission';

-- 1.8 Permission Resolution Function
CREATE OR REPLACE FUNCTION get_user_permissions_4CBV(p_user_id UUID)
RETURNS TABLE (
  permission_id UUID,
...
BEGIN
  ...
  SELECT DISTINCT
    ...
    rp.constraints
  FROM user_roles_4CBV ur
  JOIN roles_4CBV r ON ur.role_id = r.id
  JOIN role_permissions_4CBV rp ON r.id = rp.role_id
  JOIN permissions_4CBV p ON rp.permission_id = p.id
  JOIN modules_4CBV m ON p.module_id = m.id
  ...
  SELECT DISTINCT
    ...
    uap.constraints
  FROM user_additional_permissions_4CBV uap
  JOIN permissions_4CBV p ON uap.permission_id = p.id
  JOIN modules_4CBV m ON p.module_id = m.id
...
COMMENT ON FUNCTION get_user_permissions_4CBV IS 'Returns all effective permissions for a user from role and additional permissions';
...
CREATE INDEX idx_doubts_student ON doubts_4CBV(student_id);
CREATE INDEX idx_doubts_subject ON doubts_4CBV(subject_id);
CREATE INDEX idx_doubts_section ON doubts_4CBV(section_id);
CREATE INDEX idx_doubts_status ON doubts_4CBV(status);
CREATE INDEX idx_doubts_assigned_teacher ON doubts_4CBV(assigned_teacher_id);
CREATE INDEX idx_doubts_number ON doubts_4CBV(doubt_number);
CREATE INDEX idx_doubts_created_at ON doubts_4CBV(created_at DESC);

COMMENT ON TABLE doubts_4CBV IS 'Student academic questions and answers system';
COMMENT ON COLUMN doubts_4CBV.is_public IS 'If true, visible to other students in same class';
COMMENT ON COLUMN doubts_4CBV.doubt_number IS 'Format: DBT-YYYY-NNNN';

-- 10.2 Doubt Follow-ups (Threaded Discussion)
CREATE TABLE doubt_follow_ups_4CBV (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_doubt_followups_doubt ON doubt_follow_ups_4CBV(doubt_id);
CREATE INDEX idx_doubt_followups_user ON doubt_follow_ups_4CBV(user_id);
CREATE INDEX idx_doubt_followups_created_at ON doubt_follow_ups_4CBV(created_at);

COMMENT ON TABLE doubt_follow_ups_4CBV IS 'Follow-up messages for doubts - enables threaded discussion';

-- 10.3 Grievances (Complaint/Feedback System)
CREATE TABLE grievances_4CBV (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_grievances_raised_by ON grievances_4CBV(raised_by_user_id);
CREATE INDEX idx_grievances_student ON grievances_4CBV(student_id);
CREATE INDEX idx_grievances_category ON grievances_4CBV(grievance_category);
CREATE INDEX idx_grievances_status ON grievances_4CBV(status);
CREATE INDEX idx_grievances_assigned_to ON grievances_4CBV(assigned_to);
CREATE INDEX idx_grievances_number ON grievances_4CBV(grievance_number);
CREATE INDEX idx_grievances_priority ON grievances_4CBV(priority);
CREATE INDEX idx_grievances_created_at ON grievances_4CBV(created_at DESC);
CREATE INDEX idx_grievances_confidential ON grievances_4CBV(is_confidential) WHERE is_confidential = true;

COMMENT ON TABLE grievances_4CBV IS 'Complaint and feedback management system for all user types';
COMMENT ON COLUMN grievances_4CBV.is_anonymous IS 'If true, complainant identity hidden from non-admin roles';
COMMENT ON COLUMN grievances_4CBV.is_confidential IS 'If true, only accessible to Principal and assigned person';
COMMENT ON COLUMN grievances_4CBV.escalation_level IS '0=Normal, 1=Principal, 2=Management';
COMMENT ON COLUMN grievances_4CBV.grievance_number IS 'Format: GRV-YYYY-NNNN';

-- 10.4 Grievance Updates (Progress Tracking)
CREATE TABLE grievance_updates_4CBV (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_grievance_updates_grievance ON grievance_updates_4CBV(grievance_id);
CREATE INDEX idx_grievance_updates_updated_by ON grievance_updates_4CBV(updated_by);
CREATE INDEX idx_grievance_updates_created_at ON grievance_updates_4CBV(created_at);

COMMENT ON TABLE grievance_updates_4CBV IS 'Tracks all updates and communications for grievances';
COMMENT ON COLUMN grievance_updates_4CBV.is_visible_to_complainant IS 'Internal notes can be hidden from complainant';

-- ============================================================================
-- 11. FOREIGN KEY CONSTRAINTS
-- ============================================================================

ALTER TABLE roles_4CBV ADD CONSTRAINT fk_roles_created_by FOREIGN KEY (created_by) REFERENCES users_4CBV(id) ON DELETE SET NULL;
