...
CREATE TABLE users_3AAA (
  ...
  primary_role_id UUID REFERENCES roles_3AAA(id) ON DELETE SET NULL,
  is_active BOOLEAN DEFAULT true,
...
CREATE INDEX idx_users_primary_role ON users_3AAA(primary_role_id) WHERE deleted_at IS NULL;

COMMENT ON TABLE users_3AAA IS 'User management table - primary_role_id for quick access, additional roles via user_roles_3AAA';
COMMENT ON COLUMN users_3AAA.auth_user_id IS 'Links to Supabase auth.users table';
COMMENT ON COLUMN users_3AAA.primary_role_id IS 'Primary role for quick access during login - synced with user_roles_3AAA';
...
CREATE INDEX idx_doubts_student ON doubts_3AAA(student_id);
CREATE INDEX idx_doubts_subject ON doubts_3AAA(subject_id);
CREATE INDEX idx_doubts_section ON doubts_3AAA(section_id);
CREATE INDEX idx_doubts_status ON doubts_3AAA(status);
CREATE INDEX idx_doubts_assigned_teacher ON doubts_3AAA(assigned_teacher_id);
CREATE INDEX idx_doubts_number ON doubts_3AAA(doubt_number);
CREATE INDEX idx_doubts_created_at ON doubts_3AAA(created_at DESC);

COMMENT ON TABLE doubts_3AAA IS 'Student academic questions and answers system';
COMMENT ON COLUMN doubts_3AAA.is_public IS 'If true, visible to other students in same class';
COMMENT ON COLUMN doubts_3AAA.doubt_number IS 'Format: DBT-YYYY-NNNN';

-- 10.2 Doubt Follow-ups (Threaded Discussion)
CREATE TABLE doubt_follow_ups_3AAA (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_doubt_followups_doubt ON doubt_follow_ups_3AAA(doubt_id);
CREATE INDEX idx_doubt_followups_user ON doubt_follow_ups_3AAA(user_id);
CREATE INDEX idx_doubt_followups_created_at ON doubt_follow_ups_3AAA(created_at);

COMMENT ON TABLE doubt_follow_ups_3AAA IS 'Follow-up messages for doubts - enables threaded discussion';

-- 10.3 Grievances (Complaint/Feedback System)
CREATE TABLE grievances_3AAA (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_grievances_raised_by ON grievances_3AAA(raised_by_user_id);
CREATE INDEX idx_grievances_student ON grievances_3AAA(student_id);
CREATE INDEX idx_grievances_category ON grievances_3AAA(grievance_category);
CREATE INDEX idx_grievances_status ON grievances_3AAA(status);
CREATE INDEX idx_grievances_assigned_to ON grievances_3AAA(assigned_to);
CREATE INDEX idx_grievances_number ON grievances_3AAA(grievance_number);
CREATE INDEX idx_grievances_priority ON grievances_3AAA(priority);
CREATE INDEX idx_grievances_created_at ON grievances_3AAA(created_at DESC);
CREATE INDEX idx_grievances_confidential ON grievances_3AAA(is_confidential) WHERE is_confidential = true;

COMMENT ON TABLE grievances_3AAA IS 'Complaint and feedback management system for all user types';
COMMENT ON COLUMN grievances_3AAA.is_anonymous IS 'If true, complainant identity hidden from non-admin roles';
COMMENT ON COLUMN grievances_3AAA.is_confidential IS 'If true, only accessible to Principal and assigned person';
COMMENT ON COLUMN grievances_3AAA.escalation_level IS '0=Normal, 1=Principal, 2=Management';
COMMENT ON COLUMN grievances_3AAA.grievance_number IS 'Format: GRV-YYYY-NNNN';

-- 10.4 Grievance Updates (Progress Tracking)
CREATE TABLE grievance_updates_3AAA (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_grievance_updates_grievance ON grievance_updates_3AAA(grievance_id);
CREATE INDEX idx_grievance_updates_updated_by ON grievance_updates_3AAA(updated_by);
CREATE INDEX idx_grievance_updates_created_at ON grievance_updates_3AAA(created_at);

COMMENT ON TABLE grievance_updates_3AAA IS 'Tracks all updates and communications for grievances';
COMMENT ON COLUMN grievance_updates_3AAA.is_visible_to_complainant IS 'Internal notes can be hidden from complainant';
