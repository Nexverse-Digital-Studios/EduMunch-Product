              ...
              {batches.map((batch) => (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 border-b border-gray-100 hover:bg-gray-50 transition-colors ${
                    selectedBatch === batch.id ? 'bg-blue-50 border-l-4 border-l-blue-500' : ''
                  }`}
                >
            ...
            {currentBatch ? (
              <div>
                ...
                {activeTab === 'subjects' && (
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Add Subject</label>
                    ...
                    <div>
                      <h4 className="font-semibold text-gray-900 mb-3">Assigned Subjects ({batchSubjects.length})</h4>
                      <div className="space-y-2">
                        {batchSubjects.length === 0 ? (
                          <p className="text-gray-500">No subjects assigned</p>
                        ) : (
                          batchSubjects.map((bs: any) => (
                            <div key={bs.id} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                              <div>
                                <div className="font-semibold text-gray-900">{bs.subject?.name}</div>
                                <div className="text-sm text-gray-600">{bs.subject?.code}</div>
                              </div>
                              <button
                ...
                {activeTab === 'faculty' && (
                  <div className="space-y-4">
                    <div>
                      <h4 className="font-semibold text-gray-900 mb-3">Assigned Faculty ({batchFaculty.length})</h4>
                      <div className="space-y-2">
                        {batchFaculty.length === 0 ? (
                          <p className="text-gray-500">No faculty assigned</p>
                        ) : (
                          batchFaculty.map((bf: any) => (
                            <div key={bf.id} className="p-3 bg-gray-50 rounded-lg">
                              <div className="flex justify-between">
                                <div>
                                  <div className="font-semibold text-gray-900">
                                    {bf.faculty?.first_name} {bf.faculty?.last_name}
                                  </div>
                                  <div className="text-sm text-gray-600">{bf.subject?.name}</div>
                                </div>
              ...
              <div className="text-center py-12 text-gray-500">No batch selected</div>
            )}
          </div>
        </div>
      )}
