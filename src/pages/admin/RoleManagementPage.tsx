  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {loading ? (
          <div className="col-span-full text-center py-8 text-gray-400">Loading roles...</div>
        ) : roles.length === 0 ? (
          <div className="col-span-full text-center py-8 text-gray-400">No roles found</div>
        ) : (
          roles.map((role) => {
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <Trash2 size={18} className={role.is_system_role ? 'text-gray-600' : 'text-red-400'} />
                    </button>
                  </div>
                </div>
                ...
                <div className="mt-4 pt-4 border-t border-gray-700">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-lg font-bold text-indigo-400">{role.permission_count}</span>
                  </div>
                  <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8">
        <div className="rounded-lg border p-6" style={{ backgroundColor: '#1f2937', borderColor: uiConfig.colors.primary['500'] }}>
          <p className="text-gray-400 text-sm mb-2">Total Roles</p>
          <p className="text-3xl font-bold text-white">{roles.length}</p>
        </div>
        <div className="rounded-lg border p-6" style={{ backgroundColor: '#1f2937', borderColor: uiConfig.colors.primary['500'] }}>
          <p className="text-gray-400 text-sm mb-2">System Roles</p>
          <p className="text-3xl font-bold text-blue-400">{roles.filter(r => r.is_system_role).length}</p>
        </div>
        <div className="rounded-lg border p-6" style={{ backgroundColor: '#1f2937', borderColor: uiConfig.colors.primary['500'] }}>
          <p className="text-gray-400 text-sm mb-2">Custom Roles</p>
          <p className="text-3xl font-bold text-purple-400">{roles.filter(r => r.is_custom_role).length}</p>
        </div>
      </div>
    </div>
  );
